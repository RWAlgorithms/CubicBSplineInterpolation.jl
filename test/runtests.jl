...
@testset "1D test" begin
    ...
    ϵ = T(1.0e-8) # coefficient tolerance.
    N = 100
    a = T(-10)
    b = T(3.4)
...
# # Tests for Interpolation-only usage
...
# based on interpolate_only.jl
@testset "interpolation-only tests" begin

    T = Float64
    rng = Random.Xoshiro(0)

    derivative_rtol = T(1.0e-8)
    ...
    for x0 in x0_set
        ...
        for _ in 1:num_test_per_position
            ...
            @test norm(dq_c_FD - dq_c) / norm(dq_c_FD) < derivative_rtol
        end
    end
end
