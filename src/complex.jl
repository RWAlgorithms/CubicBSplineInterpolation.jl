...
struct Interpolator1DComplex{T<:AbstractFloat} <: AbstractInterpolator1D
    ...
    function Interpolator1DComplex(
        ...
        _get_coeffs!(padding_option, extrapolation_option, c_real, buf, s_real, tmp_r, ϵ)

        c_imag = Memory{T}(undef, get_num_coeffs(buf))
        _get_coeffs!(padding_option, extrapolation_option, c_imag, buf, s_imag, tmp_r, ϵ)
...
function update_itp!(
    ...
    _get_coeffs!(padding_option, extrapolation_option, itp.real_coeffs, buf, s_real, LinRange(x_start, x_fin, length(s_real)), ϵ)
    _get_coeffs!(padding_option, extrapolation_option, itp.imag_coeffs, buf, s_imag, LinRange(x_start, x_fin, length(s_imag)), ϵ)
    return nothing
end
...
struct Interpolator2DComplex{T<:AbstractFloat} <: AbstractInterpolator2D
    ...
    #     _get_coeffs!(Yr, X, buf1, buf2, S_real, ϵ)

    #     Yi = zeros(T, size(S_imag))
    #     _get_coeffs!(Yi, X, buf1, buf2, S_real, ϵ)
    ...
    function Interpolator2DComplex(
        ...
        _get_coeffs!(padding_option, extrapolation_option, c_real, buf, S_real, xs1, xs2, ϵ)

        c_imag = zeros(T, get_coeffs_size(buf)) # allocates.
        _get_coeffs!(padding_option, extrapolation_option, c_imag, buf, S_imag, xs1, xs2, ϵ)
...
function update_itp!(
    ...
    _get_coeffs!(padding_option, extrapolation_option, itp.real_coeffs, buf, S_real, xs1, xs2, ϵ)
    _get_coeffs!(padding_option, extrapolation_option, itp.imag_coeffs, buf, S_imag, xs1, xs2, ϵ)
    return nothing
end
