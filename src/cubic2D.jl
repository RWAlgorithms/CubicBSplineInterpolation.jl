...
struct Interpolator2D{T <: AbstractFloat} <: AbstractInterpolator2D{T}
    coeffs::Matrix{T}
    ...
    function Interpolator2D(S::Matrix{T}, x1_start::T, x1_fin::T, x2_start::T, x2_fin::T; ϵ::T = eps(T)*100) where T <: AbstractFloat
        ...
        return new{T}(c, A1, A2)
    end
end

function get_query_interval(itp::AbstractInterpolator2D)
...
struct Interpolator2DComplex{T <: AbstractFloat} <: AbstractInterpolator2D{T}
    ...
    function Interpolator2DComplex(S_real::Matrix{T}, S_imag::Matrix{T}, x1_start::T, x1_fin::T, x2_start::T, x2_fin::T; ϵ::T = eps(T)*100) where T <: AbstractFloat
        ...
        return new{T}(c_real, c_imag, A1, A2)
    end
end

function get_query_interval(itp::Interpolator2DComplex)
    x1_lb, x1_ub = get_query_interval(itp.x1_query_cache, size(itp.real_coeffs,1))
    x2_lb, x2_ub = get_query_interval(itp.x2_query_cache, size(itp.real_coeffs,2))
    return x1_lb, x1_ub, x2_lb, x2_ub
end
