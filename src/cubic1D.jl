...
function _get_coeffs!(
    ...
    # the following condition won't work with view of a multi-dim array. Make this function internal and skip the stride test.
    ...
    #_get_coeffs_1d!(c_minus, buf.c_plus, s_ext, ϵ)
    _get_coeffs_1d!(c_minus, c_plus, s_ext, ϵ)
...
function _get_coeffs!(c_minus::Union{Memory{T},Vector{T}}, s::AbstractVector{T}, ϵ::T) where {T<:AbstractFloat}
    ...
    _get_coeffs_1d!(c_minus, c_plus, s, ϵ)
    return nothing
end
...
struct Interpolator1D{T<:AbstractFloat} <: AbstractInterpolator1D
    ...
    function Interpolator1D(
        ...
        _get_coeffs!(pading_option, extrapolation_option, c, buf, s, tmp_r, ϵ)
...
function update_itp!(padding_option::PaddingOption, extrapolation_option::ExtrapolationOption, itp::Interpolator1D, buf::FitBuffer1D, s::AbstractVector{T}; ϵ::T=eps(T) * 2) where {T<:AbstractFloat}
    ...
    _get_coeffs!(padding_option, extrapolation_option, itp.coeffs, buf, s, LinRange(x_start, x_fin, length(s)), ϵ)
    return nothing
end
